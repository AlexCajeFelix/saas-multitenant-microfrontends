...
import type { ChangePlanResult } from "../../api/types";
import { useTenant } from "../../lib/tenant";
import { useApiMutation } from "../../lib/mutations";
import { formatDate, formatMoney, SUBSCRIPTION_STATUS_LABEL } from "../../lib/format";
...
export function SubscriptionPage() {
  ...
  const invalidate = [["subscription", tenantId], ["invoices", tenantId]];
  ...
  const others = (plans.data ?? []).filter((option) => option.code !== plan.code && option.isActive);
          ...
          <Section title="Plano atual">
            <div className="flex flex-wrap items-start justify-between gap-4 p-4">
              ...
              <p className="text-right text-2xl font-semibold tracking-tight text-slate-900">
                {plan.priceFormatted}
                <span className="block text-xs font-normal text-slate-500">por {plan.interval}</span>
              </p>
            </div>
