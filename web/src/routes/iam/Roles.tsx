    ...
    <>
      ...
      >
        <div className="grid gap-3 sm:grid-cols-2">
          <Field label="Slug" hint="minusculas, sem espaco" error={fieldError(create.error, "slug")}>
            <Input
              required
              autoFocus
              value={slug}
            ...
            {roles.map((role) => (
              <Tr key={role.id}>
                <Td>
                  ...
                  {role.description && (
                    <span className="block truncate text-xs text-slate-500">{role.description}</span>
                  )}
                </Td>
                <Td>
