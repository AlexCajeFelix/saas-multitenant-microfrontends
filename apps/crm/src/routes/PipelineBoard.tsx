...
import { formatDate, formatMoney } from "@saas/platform/lib/format";
import { PageHeader } from "@saas/platform/components/PageHeader";
import { Badge } from "@saas/platform/components/ui/Badge";
import { PermissionButton } from "@saas/platform/components/ui/Button";
import { Field, Textarea } from "@saas/platform/components/ui/Field";
import { FormDialog } from "@saas/platform/components/ui/Dialog";
import { Spinner, QueryError, EmptyState } from "@saas/platform/components/ui/States";
      ...
      <div className="flex min-h-24 flex-1 flex-col gap-2 px-2 pb-2">
        {deals.map((deal) => (
          <DraggableCard
            key={deal.id}
            deal={deal}
            draggable={draggable && deal.status === "open"}
          />
        ))}
        {deals.length === 0 && (
...
function Card({
  ...
  return (
    <article className={`card p-2.5 ${overlay ? "rotate-2 shadow-lg" : "hover:border-slate-300"}`}>
